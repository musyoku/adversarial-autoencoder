...
for epoch in xrange(1, max_epoch + 1):
	...
	for t in xrange(1, num_trains_per_epoch + 1):
		...
		for k in xrange(n_steps_to_optimize_dis):
			if k > 0:
				x, y, label_ids = sample_data()
			# z_true = sampler.supervised_gaussian_mixture(batchsize, conf.ndim_z, label_ids.data, n_labels)
			z_true = sampler.supervised_swiss_roll(batchsize, conf.ndim_z, label_ids.data, n_labels)
			loss_discriminator += aae.train_discriminator_z(x, y, z_true)
		loss_discriminator /= n_steps_to_optimize_dis
		sum_loss_discriminator += loss_discriminator

		# train generator
		sum_loss_generator = aae.train_generator_x_z(x, y)
